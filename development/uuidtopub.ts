...
const uuidToPublicId = <const T extends keyof typeof idTypes>(prefix: T, input: string) => {
  ...
  return TypeID.fromUUID(prefixValue, input).asType(prefixValue).toString() as unknown as TypeId<T>;
};

const uuid = "01930c42-589d-711e-89eb-336891eb18a2";

logger.log("🚀 ~ uuidToPublicId ~ uuidToPublicId:", uuidToPublicId("account", uuid));
